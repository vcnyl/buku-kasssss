...
<div class="container">
    ...
    <form action="{{ route('superadmin.penerimaan.update', $penerimaan->id_penerimaan) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        ...
        <div class="form-group">
            ...
            <select name="id_user" id="id_user" class="form-control" required>
                @foreach($adminUsers as $user)
                    <option value="{{ $user->id_user }}" {{ $user->id_user == $penerimaan->id_user ? 'selected' : '' }}>{{ $user->nama }}</option>
                @endforeach
            </select>
        </div>
...
<script>
    ...
    document.addEventListener("DOMContentLoaded", function () {
        ...
        const rawValue = "{{ $penerimaan->nominal }}";
